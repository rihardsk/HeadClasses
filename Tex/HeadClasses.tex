...
Strict hierarchies of classes have been shown for both one-way multi-head deterministic and nondeterministic automata with regard to the head count of the automata \citep{Holzer2009,Yao1978}. In 1978 \citet{Yao1978} showed that in order to separate the class of languages that can be recognized by a $1dfa(k)$ from that which can be recognized by a $1dfa(k+1)$ it is sufficient to use the language
\[
	L'_k = \left\{w_1\$w_2\$ \ldots \$w_{2k} \middle| w_i \in \left\{a,b\right\}^* \wedge w_i = w_{2n+1-i} \textrm{ for all } 1 \leq i \leq n \right\}.
\]
