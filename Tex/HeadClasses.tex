...
\begin{definition}
A finite one-way $p$-ultrametric automata ($1u_pfa$ or $1u_pfa(1)$) is a sextuple
...
\begin{multline*}
	L(M)=\left\{ w \in \Sigma^* | \left(w,s_0,\left(0,\ldots,0\right)\right) \vdash^* \left(w,s,\left(p_1,\ldots,p_k\right)\right), s \in F,\right.\\
	\left.\textrm{and } M \textrm{ stops } \left(w,s,\left(p_1,\ldots,p_k\right)\right)\right\}.
\end{multline*}
If for any state and $k$-tuple of symbols the transition function $\delta$ assigns at most $1$ successor state 
...
\sum_{\mathclap{\substack{ x \in S \wedge \\
		...
		(p_1, \ldots, p_k)}}}
	{s_{i-1}(x, (p'_1, \ldots, p'_k)) \cdot \delta ((w_{p'_1}, \ldots, w_{p'_k}), x, y)_1 }
\]
