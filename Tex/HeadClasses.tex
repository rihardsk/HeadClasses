...
\begin{definition}
A finite $k$-head two-way $p$-ultrametric automaton ($2u_pfa(k)$) is a septuple $\langle S, \Sigma, k, s_0, \delta, Q_A, Q_R \rangle$ where
\begin{itemize}
...
Take $L \in \widehat{U_pTM}$ for some prime $p$. Lemma \ref{skaititaji} implies that there exists $m \in \mathbb{N}$ such that $f_m(L) \in \widehat{2U_pFA(3)}$. Consequently, $f_m(L) \in \widehat{2U_pFA(h)}$. Lemma \ref{plus1} implies that if $m > h + 1$, then $f_{m-1}(L) \in \widehat{2U_pFA(h + 1)} = \widehat{2U_pFA(h)}$.
% Take $m = m - 1$ 
Reduce $m$ by $1$ and repeat until we get $f_m(L) \in \widehat{2U_pFA(h)}$ and $m = h + 1$. Lemma \ref{reizinajums} implies that if $f_m(L) \in \widehat{2U_pFA(h)}$, then $L \in \widehat{2U_pFA(h \cdot m)} = \widehat{2U_pFA(h \cdot (h + 1))}$. Contradiction with Theorem \ref{atdalisana}. \qed
\end{proof}
\begin{corollary}
