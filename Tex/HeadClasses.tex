...
\begin{definition}
By $\widehat{C}$ we denote the subset of a language class $C$ containing only the words in the form $1^{2^n}, n \in \mathbb{N}$, or more precisely -- $\widehat{C} = \left\{ h | h \in C \wedge \exists n \in \mathbb{N} : h = 1^{2^n} \right\}$\\
\end{definition}
