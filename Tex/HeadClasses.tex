...
Ultrametric finite automata and ultrametric Turing machines were first introduced by \citet{Freivalds2012}. This has been followed by several papers where various aspects of them are studied in depth. \citet{ KasparsBalodis2013} has studied the descriptional complexity of ultrametric automata. He shows show that ultrametric automata can achieve an exponential advantage in terms of the number of states required when compared to equivalent deterministic automata. \citet{Krislauks2013} has studied the reversal complexity of ultrametric Turing machines.
...
\begin{itemize}
	...
	\item $\delta: Q \times \Gamma^k \times Q \times \left( \Gamma \times \{-1,0,1\} \right)^k \rightarrow \mathbb{Q}_p $ is a partially defined transition function where $-1$ denotes moving the head to the left, $1$ -- to the right and $0$ -- not moving the head and $\mathbb{Q}_p$ is the amplitude with which the transition is made.
\end{itemize}
\end{definition}
